...
const Start = function(args, pkg) {
  ...
  co(()=>new Promise((resolve, reject)=>{
    fs.readFile(cwd + '/.af-theme', 'utf8', function(error, data){
      if (error) {
        console.log('Error: Cannot open \'.af-theme\''['red']);
        console.log('Try to run \'ant-forest init\''['red']);
        process.exit();
      } else {
        ...
        try {
          ...
          console.log('Read \'.af-theme\' failed.'['red']);
